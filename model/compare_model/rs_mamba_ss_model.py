...
class SelectiveScanMamba(torch.autograd.Function):
    ...
    @staticmethod # 这个方法不需要self参数
    @torch.cuda.amp.custom_fwd # 用于自动混合精度(AMP)训练
...
'''修改方向在这里修改'''
# CrossScan将图像按8个方向展平成一维序列
# 在反向传播时，将各方向的梯度合并回原始形状
class CrossScan(torch.autograd.Function):
    ...
    def forward(ctx, x: torch.Tensor):
        ...
        # 添加横向和竖向的扫描
        xs[:, 0] = x.flatten(2, 3) # 水平方向
        xs[:, 1] = x.transpose(dim0=2, dim1=3).flatten(2, 3) # 垂直方向
        xs[:, 2:4] = torch.flip(xs[:, 0:2], dims=[-1]) # # 水平和垂直的反向
    
        # 提供斜向和反斜向的扫描
        xs[:, 4] = diagonal_gather(x)  # 主对角线方向
        xs[:, 5] = antidiagonal_gather(x) # 反对角线方向
        xs[:, 6:8] = torch.flip(xs[:, 4:6], dims=[-1]) # 主对角线和反对角线的反向
...
# CrossMerge则将各个方向处理后的结果合并回原始的图像形状
...
# 多方向选择性扫描的核心，它将图像按8个方向拆分，
# 分别执行选择性扫描，然后将结果合并

# OSSM的核心，它实现了多方向的选择性扫描操作
def cross_selective_scan(
    ...
    # ==============================
    SelectiveScan=None, # 这里我们传入的是混合精度扫描类
    CrossScan=CrossScan,
    ...
    # 在这里已经打成了一维的序列
    xs = CrossScan.apply(x) # [1, 8, 192, 16384]

    x_dbl = torch.einsum("b k d l, k c d -> b k c l", xs, x_proj_weight)
    # print("x_dbl的维度是{}".format(x_dbl.shape)) # [1, 8, 38, 16384]
    ...
    # [1, 8, 192, 16384]
    xs = xs.view(B, -1, L)
    # [1, 1536, 16384] 去除第二个维度，自动计算
    # 将8个方向和192个特征维度合并成一个维度

    # [B, K*D, L] = [1, 8*192, 16384] = [1, 1536, 16384]
    dts = dts.contiguous().view(B, -1, L)
    # [B, K*D, L] = [1, 8*192, 16384] = [1, 1536, 16384]
    As = -torch.exp(A_logs.to(torch.float)) # (k * c, d_state)
    # [K*D, N] = [8*192, 16] = [1536, 16]
    Bs = Bs.contiguous()
    # [B, K, N, L] = [1, 8, 16, 16384]
    Cs = Cs.contiguous()
    # [B, K, N, L] = [1, 8, 16, 16384]
    ...
    if out_norm_shape in ["v1"]: # (B, C, H, W)
        y = out_norm(y.view(B, -1, H, W)).permute(0, 2, 3, 1) # (B, H, W, C) #默认不运行
    else: # (B, L, C) [1, 192, 16384]
        y = y.transpose(dim0=1, dim1=2).contiguous() # (B, L, C) 交换维度1和2 [1, 16384, 192]
        y = out_norm(y).view(B, H, W, -1) # 张量重塑为 [1, 128, 128, 192]
...
# 全方向选择性扫描模块是模型的核心组件-->输入和输出相同维度
# 主要功能：对输入特征图进行全方向（8个方向）的选择性状态空间扫描
class OSSM(nn.Module):
    def __init__(
        ...
        d_model=96,                     # 输入特征维度
        d_state=16,                     # 状态空间维度
        ssm_ratio=2.0,                  # 内部维度扩展比例
        dt_rank="auto",                 # ∆t投影矩阵的秩
        act_layer=nn.SiLU,              # 激活函数
        # dwconv =============== 深度卷积参数
        d_conv=3, # < 2 means no conv   # 卷积核大小，<2表示不使用卷积
        conv_bias=True,                 # 是否为卷积添加偏置
        # ======================
        dropout=0.0,                    # dropout比例
        bias=False,                     # 是否为线性层添加偏置
        # dt init ==============
        dt_min=0.001,                   # 最小dt值
        dt_max=0.1,                     # 最大dt值
        ...
        **kwargs,
        ...
        # 计算dt_rank，如果设为"auto"，则为d_model/16向上取整
        dt_rank = math.ceil(d_model / 16) if dt_rank == "auto" else dt_rank
        self.d_conv = d_conv

        # tags for forward_type ==============================
        # 处理forward_type中的后缀标志 =========================
        def checkpostfix(tag, value):
            ...
            if ret:
                value = value[:-len(tag)]
                # 返回去掉这个后缀的字符串
            return ret, value # 返回布尔值+原字符串

        # 检查是否禁用force32模式（不强制使用float32）
        self.disable_force32, forward_type = checkpostfix("no32", forward_type)
        # 检查是否禁用z通道
        self.disable_z, forward_type = checkpostfix("noz", forward_type)
        # 检查是否禁用z通道的激活函数
        self.disable_z_act, forward_type = checkpostfix("nozact", forward_type) # 返回False v2
        # print(self.disable_z_act, forward_type)

        # softmax | sigmoid | dwconv | norm ===========================
        # 选择输出归一化的类型：softmax | sigmoid | dwconv | norm =============
        ...
        # forward_type debug =======================================
        # 定义各种前向传播实现方案 我们使用的是v2
        FORWARD_TYPES = dict(
            v0=self.forward_corev0,
            # v2使用SelectiveScanCore，强制使用float32
        ...
        # 选择forward_core函数
        self.forward_core = FORWARD_TYPES.get(forward_type, None) # 这里返回的应该是v2的patical部分参数函数
        ...
        # in proj =======================================
        # 如果disable_z为False，则投影维度翻倍（用于门控机制）
        d_proj = d_inner if self.disable_z else (d_inner * 2)
        # 输入投影层：d_model -> d_proj
        ...
        # conv =======================================
        if d_conv > 1:
            # 如果d_conv>1，添加深度卷积层处理空间信息
            self.conv2d = nn.Conv2d(
                in_channels=d_inner,
                out_channels=d_inner,
                groups=d_inner,                     # 深度卷积，每个通道独立卷积
                bias=conv_bias,
                kernel_size=d_conv,
                padding=(d_conv - 1) // 2,          # 保持空间尺寸不变
                **factory_kwargs,
            )
    ...
    def dt_init(dt_rank, d_inner, dt_scale=1.0, dt_init="random", dt_min=0.001, dt_max=0.1, dt_init_floor=1e-4, **factory_kwargs):
        ...
        # 初始化dt偏置，使得F.softplus(dt_bias)在dt_min和dt_max之间
        dt = torch.exp(
    ...
    def forward_corev2(self, x: torch.Tensor, channel_first=False, SelectiveScan=SelectiveScanOflex, cross_selective_scan=cross_selective_scan, force_fp32=None):
        ...
        '''
        Args:
            x: 前线传播的时候传入了                               forword
            channel_first: 是否为通道优先  BCHW 格式 还是 BHWC 格式 forword
            SelectiveScan: SelectiveScanCore                    partical
            cross_selective_scan:                               默认 实际未传入
            force_fp32: True                                    partical

        Returns:

        '''
        if not channel_first:
            x = x.permute(0, 3, 1, 2).contiguous() # 默认不执行
        # ZSJ V2版本使用的mamba，要改扫描方向在这里改
        # print("输入OSSM之前x的形状{}".format(x.shape)) # [1, 192, 128, 128]
        B, D, H, W = x.shape
        ...
        # print("输入OSSM之后x的形状{}".format(x.shape)) # [1, 128, 128, 192]
        # [1, 128, 128, 192]
        return x
    ...
    def forward(self, x: torch.Tensor, **kwargs):
        ...
        if not self.disable_z:
            ...
            # print('{}'.format(x.shape)) # [1, 128, 128, 192]
        ...
        if with_dconv:
            # 转换为channel_first格式
            x = x.permute(0, 3, 1, 2).contiguous() # [1,192,128,128]
            # print('调整维度顺序{}'.format(x.shape)) # [1, 192, 128, 128]
            # 应用深度卷积
            x = self.conv2d(x) # (b, d, h, w)
            # print('深度卷积之后{}'.format(x.shape)) # [1, 192, 128, 128]
        # 对x应用激活函数
        x = self.act(x)
        # 调用核心前向传播函数
        # 为什么这里传入的是channel_first=with_dconv
        y = self.forward_core(x, channel_first=with_dconv)
        # 如果启用z通道，将结果与z相乘（门控机制）
        if not self.disable_z:
            y = y * z
        # 输出投影和dropout
...
# 用于调整张量的维度顺序 *args传入变换位置 从[B,C,H,W]变为[B,H,W,C]
    ...
    def _forward(self, input: torch.Tensor):
        if self.ssm_branch:
            if self.post_norm:
                ...
                x = input + self.drop_path(self.op(self.norm(input))) # 先走这个
        if self.mlp_branch:
            if self.post_norm:
                x = x + self.drop_path(self.norm2(self.mlp(x))) # FFN
            else:
                x = x + self.drop_path(self.mlp(self.norm2(x))) # FFN # # 后走这个
        return x
...
class RSM_SS(nn.Module):
    def __init__(
        ...
        in_chans=3, 
        num_classes=1000, 
        depths=[2, 2, 9, 2], # 不同层级的块数量
        dims=[96, 192, 384, 768], # 每个阶段的特征维度列表
        # =========================
        ssm_d_state=16,
        ssm_ratio=2.0,
        ssm_dt_rank="auto",
        ...
        forward_type="v2",
        # =========================
        mlp_ratio=4.0, # MLP隐藏层扩展比例，默认为4.0
        mlp_act_layer="gelu", # MLP激活函数，默认为"gelu"
        mlp_drop_rate=0.0, # MLP的dropout比例，默认为0.0
        # =========================
        drop_path_rate=0.1,  # 随机深度路径丢弃率，默认为0.1
        patch_norm=True,  # 是否对分块进行归一化，默认为True
        norm_layer="LN",
        use_checkpoint=False,   # 是否使用梯度检查点技术，默认为False
        **kwargs,
    ):
        super().__init__()
        self.num_classes = num_classes
        self.num_layers = len(depths) # # 每个阶段的块数列表 求取长度为4
        # 如果dims是整数，则创建一个每层翻倍的dims列表 如果是48
        # 因为dims如果我传入的话就是整数了，如果不传入就是原先列表
        # num_layers=depths=[2, 2, 9, 2] 这个也可以在超参数里面进行调整，编码器每次堆叠的个数
        ...
        self.dims = dims
        # 创建drop path rate列表，这是一种正则化技术，从0到drop_path_rate线性增加
        ...
        for i_layer in range(self.num_layers):
            ...
            # 每个层之前（除了第一层）进行下采样 dims=[96, 192, 384, 768]
            downsample = _make_downsample(
                self.dims[i_layer - 1], # 输入维度 96
                self.dims[i_layer],     # 输出维度 192
                norm_layer=norm_layer,
            ) if (i_layer != 0) else nn.Identity()  # ZSJ 修改为i_layer != 0，也就是第一层不下采样，和论文的图保持一致，也方便我取出每个尺度处理好的特征
            # PyTorch中的一个 无操作层

            # 在encoder_layers里面添加网络结构
            self.encoder_layers.append(
                # 每个层由一个下采样操作和多个OSSBlock组成
                self._make_layer(
    ...
    def _make_patch_embed_v2(in_chans=3, embed_dim=96, patch_size=4, patch_norm=True, norm_layer=nn.LayerNorm):
        ...
        return nn.Sequential(
            # 一个3x3卷积，步长为2 通道数变成 embed_dim // 2                             # [B, 3, 512, 512]
            nn.Conv2d(in_chans, embed_dim // 2, kernel_size=3, stride=2, padding=1), # [B, 48, 256, 256]
            (Permute(0, 2, 3, 1) if patch_norm else nn.Identity()),             # [B, 256, 256, 48]
            (norm_layer(embed_dim // 2) if patch_norm else nn.Identity()),            # 应用LayerNorm到最后一个维度（48）
            (Permute(0, 3, 1, 2) if patch_norm else nn.Identity()),             #  [B, 48, 256, 256]
            nn.GELU(),                                                                  #  [B, 48, 256, 256]
            # 一个3x3卷积，步长为2 通道数增加到embed_dim
            nn.Conv2d(embed_dim // 2, embed_dim, kernel_size=3, stride=2, padding=1),# [B, 96, 128, 128]
            Permute(0, 2, 3, 1),                                               # # [B, 128, 128, 96]
            (norm_layer(embed_dim) if patch_norm else nn.Identity()),                   # [B, 128, 128, 96]
        )
    ...
    def _make_layer(
        ...
        **kwargs,
        ...
        depth = len(drop_path) # drop_path列表的长度决定了该层OSSBlock堆叠几个 每个数字代表dropout的丢失概率
                                # 每个OSSBlock会使用drop_path列表中对应位置的丢弃概率值
        blocks = [] #OSSBlock添加进入到blocks中
        ...
        # 将下采样层(downsample)和所有块打包成一个顺序模块返回
    ...
    def forward(self, x1: torch.Tensor):
        ...
        # print('经过patch_embed之后{}'.format(x1.shape)) # [1, 128, 128, 96]
...
if __name__ == '__main__':
    ...
    if device.type == 'cpu':
        print("Warning: No CUDA device found, running on CPU. This will be very slow and may fail if CUDA-specific ops are used without a CPU fallback.")

    # 根据 YAML 文件中的 model_args 创建模型实例
    model = RSM_SS(
        in_chans=3,
        num_classes=1,
        dims=[96, 192, 384, 768],
        depths=[2, 2, 9, 2],
        drop_path_rate=0.2
    ).to(device)  # <-- 1. 将模型移动到GPU
    model.eval()

    # 创建一个模拟输入张量
    # 尺寸参考 YAML 文件中的 batch_size 和 image_size
    batch_size = 2
    input_height, input_width = 1024, 1024
    input_tensor = torch.randn(batch_size, 3, input_height, input_width).to(device) # <-- 2. 将输入张量移动到GPU
