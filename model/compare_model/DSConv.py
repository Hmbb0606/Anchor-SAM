...
class DSConv(nn.Module):
    ...
    def forward(self, f):
        # If device is not set, infer from input tensor
...
class DSC(object):
    ...
    def _bilinear_interpolate_3D(self, input_feature, y, x):
        ...
        base_y0 = base + y0.long() * self.height
        base_y1 = base + y1.long() * self.height

        # top rectangle of the neighbourhood volume
        index_a0 = base_y0.long() - base.long() + x0.long()
        index_c0 = base_y0.long() - base.long() + x1.long()

        # bottom rectangle of the neighbourhood volume
        index_a1 = base_y1.long() - base.long() + x0.long()
        index_c1 = base_y1.long() - base.long() + x1.long()

        # get 8 grid values
        value_a0 = input_feature_flat[index_a0]
        value_c0 = input_feature_flat[index_c0]
        value_a1 = input_feature_flat[index_a1]
        value_c1 = input_feature_flat[index_c1]
        ...
        vol_a0 = ((y1_float - y) * (x1_float - x)).unsqueeze(-1)
        vol_c0 = ((y1_float - y) * (x - x0_float)).unsqueeze(-1)
        vol_a1 = ((y - y0_float) * (x1_float - x)).unsqueeze(-1)
        vol_c1 = ((y - y0_float) * (x - x0_float)).unsqueeze(-1)
